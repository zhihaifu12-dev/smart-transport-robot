...
/*清除舵机多圈角度*/
bool FSUS_Servo::reset_multi_angle() {
    this->protocol->emptyCache();       //清空串口缓冲区
    this->protocol->sendRest_Multi_Angle(servoId);  //发送重设多圈圈数指令
}
// 舵机标定
...
void FSUS_Servo::setAngle(FSUS_SERVO_ANGLE_T angle, FSUS_INTERVAL_T interval, FSUS_POWER_T power){
    ...
    if(!this->isMTurn){
      ...
      setRawAngleMTurn(angleReal2Raw(this->targetAngle), interval, power);
    }
}
