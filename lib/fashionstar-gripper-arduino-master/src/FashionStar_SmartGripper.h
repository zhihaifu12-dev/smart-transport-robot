...
// 爪子默认的闭合周期
#define FSGP_DEFAULT_INTERVAL 300
...
public:
    ...
    // 夹具
    FSGP_Gripper(FSUS_Servo *servo, float angleGripperOpen, float angleGripperClose,float angleGripperMax);
    // 初始化舵机
    bool init();
    ...
    void wait();
    //重设各个角度
    void set(float angleGripperOpen, float angleGripperClose,float angleGripperMax);
    // 判断当前爪子是否有负载
    bool isUnderLoad();
    // 判断爪子的负载是否掉落
    bool isLoadDropped();
    ...
    float angleGripperMax;      //爪子张开的最大角度
