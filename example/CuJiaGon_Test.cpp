...
#ifndef ARM_TWO
/*爪子*/
int GRIPPER_OPEN_ANGLE = -75;     // 爪子张开时的角度
int GRIPPER_CLOSE_ANGLE = -102;    // 爪子闭合时的角度
int GRIPPER_OPEN_MAX_ANGLE = -65; // 爪子张开最大大角度
/*基座*/
int ARM_BASE_Stepper_HOME_ANGLE = 315;             // 机械臂发车初始角度
int ARM_BASE_STEPPER_ANGLE[4] = {2180, 775, 1215, 1655}; // 机械臂底部舵机角度{storage,R,G,B}
 ...
 * @param deltaX 视觉检测到的 X 轴偏移量
 * @param deltaY 视觉检测到的 Y 轴偏移量
 */
...
// 更新夹爪步进电机的当前位置
    ...
    // 计算目标向量的 X 分量，步进向外为正，视觉向内为正
    float targetVectorX = gripperStepper.currentPosition - deltaY;
    
    // 计算目标向量的 Y 分量，舵机顺时针为正，视觉逆时针切向为正
    float targetVectorY = -deltaX;

    // 计算夹爪步进电机的目标位置
    float gripperStepperTarget = targetVectorX;

    // 计算角度偏移量，使用 atan2 函数替代近似计算，提高精度
    float deltaTheta = std::atan2(targetVectorY, targetVectorX) * (180.0 / M_PI);

    // 对夹爪步进电机的目标位置进行限位处理
    gripperStepperTarget = std::clamp(gripperStepperTarget, 0.0f, 1500.0f);

    // 计算机械臂基座步进电机的目标角度，这里假设 1 是初始角度，可根据实际情况调整
    float armBaseStepperTarget = ARM_BASE_Stepper_HOME_ANGLE + deltaTheta;

    // 控制夹爪步进电机移动到目标位置
    gripperStepper.runToNewPosition(gripperStepperTarget);
    ...
    case CuJiaGon:
        ...
        while (dataIndex <= (2 + rounds * 3))
        {
            ...
            case 0x22:
                ...
                {
                    if (fabs(Move_X_Grab) < 5 && fabs(Move_Y_Grab) < 5)
                    {
                        if (!Ready_to_Grab)
                        {
                        ...
                        } else if(armStepper.onPos_state && fabs(Move_X_Grab) == 0 && fabs(Move_Y_Grab) == 0 ){//识别准确
                                state = 0x33;
                                armStepper.runToNewPosition(STEPPER_ZERO);
                                gripperStepper.runToNewPosition(STEPPER_GRIPPER[0]);
                                armStepper.wait();
