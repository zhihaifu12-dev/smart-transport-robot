        ...
        while (dataIndex <= (2 + rounds * 3))
        {
            ...
            case 0x22:  
                ...
                if (vision_updated && MaixCam.Color == qr_int_str[dataIndex])
                {
                    ...
                    /*********************************************************/
                    if (fabs(Move_X_Grab) < 5 + isOpen * 5 && Move_Y_Grab < 8 + isOpen * 5 && !gripper_state && (!Ready_to_Grab || !armStepper.onPos_state)) // 未夹持时（上方和下降中）允许物料的范围
                    {
                        /*机械臂下降*/
                        if (!Ready_to_Grab)
                        {
        ...
        while (dataIndex <= (2 + rounds * 3))
        {
            ...
            case 0x22: // 脚校准
                ...
                if (vision_updated && MaixCam.Color == Green)
                {
                    ...
                    Follow_bySpeed(1.5, 75, 5, Move_X_Grab, Move_Y_Grab);
                    vision_updated = false;
                }
                break;
            case 0x33: // 手校准----蓝色
