...
CAR_GOAL_POINT MoveSquence[] =
{
    ...
    {750, 150, 0, 400, 2, 200, 2},           // 1扫码
    {1400, 50, 0, 400, 2, 200, 2},           // 2抓取
    {1050, 200, -M_PI / 2, 400, 2, 200, 2},  // 3路口旋转
    {1050, 1700, -M_PI / 2, 400, 2, 200, 2}, // 4粗加工区
    {1050, 1850, M_PI, 400, 2, 200, 2},     // 5粗加工中心
    {1850, 1850, M_PI / 2, 400, 2, 200, 2},  // 6拐角1
    {1900, 1020, M_PI / 2, 400, 2, 200, 2},  // 7存储区中心
    {1850, 150, 0, 400, 2, 50, 2},           // 8拐角2
    {-50, -50, 0, 400, 2, 200, 2}            // 9原点
}; // 每次移动的距离，基于局部坐标
...
void setup()
{
    ...
    FollowX_PID.PID_Init(Follow_PID_Kp, Follow_PID_Ki, Follow_PID_Kd, Follow_PID_MItg, Follow_PID_MOut);
    FollowY_PID.PID_Init(Follow_PID_Kp, Follow_PID_Ki, Follow_PID_Kd, Follow_PID_MItg, Follow_PID_MOut);
        ...
        while (dataIndex <= (2 + rounds * 3))
        {
            ...
            case 0x22:
                ...
                if (vision_updated && MaixCam.Color == qr_int_str[dataIndex])
                {
                    ...
                    /*********************************************************/
                    if (fabs(Move_X_Grab) < 5 && Move_Y_Grab < 5 && !gripper_state && (!Ready_to_Grab || !armStepper.onPos_state)) // 未夹持时（上方和下降中）允许物料的范围
                    {
                        /*机械臂下降*/
                        if (!Ready_to_Grab)
                        {
                    ...
                    else if (Ready_to_Grab && armStepper.onPos_state && fabs(Move_X_Grab) < 5 && Move_Y_Grab > -8)
                    { // 下降完成（包含夹持时）后阈值范围需要变化
                        Current_Time = millis();
                    }
                    else
                    {
                        if (gripper_state && fabs(Move_Y_Grab) > 8)
    ...
    case CuJiaGon:
        ...
        while (dataIndex <= (2 + rounds * 3))
        {
            ...
            case 0x22: // 脚校准
                ...
                {
                    if (fabs(Move_X_Grab) > 0 * Scale || fabs(Move_Y_Grab) > 0 * Scale)
                    {
                        Previous_Time = millis();
                    }
                    if (fabs(Move_X_Grab) < 10 && fabs(Move_Y_Grab) < 10)
                    {
                        if (!Ready_to_Grab)
                        {
                        ...
                        else if (armStepper.onPos_state && fabs(Move_X_Grab) <=  0 * Scale && fabs(Move_Y_Grab) <= 0 * Scale)
                        { // 识别准确
                            Current_Time = millis();
    ...
    case Move:
        ...
        if (CarState == Rotate_Complete)
        {
            ...
            case 9:
                ...
                break;
            default:
                if (MoveIndex == 4 || MoveIndex == 6) // 任务前一个点位
                {
                    armBaseStepper.setAngle(ARM_BASE_STEPPER_ANGLE[2]);
                    storageServo.setAngle(storage[qr_int_str[rounds * 3]]);
                    MaixCam.Maix_Detect(Green);
