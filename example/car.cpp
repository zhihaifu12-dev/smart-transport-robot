        ...
        while (dataIndex <= (2 + rounds * 3))
        {
            ...
            case 0x22:
                ...
                if (vision_updated && MaixCam.Color == qr_int_str[dataIndex])
                {
                    ...
                    /*********************************************************/
                    if (fabs(Move_X_Grab) < 5 && Move_Y_Grab < 8 && !gripper_state && (!Ready_to_Grab || !armStepper.onPos_state)) // 未夹持时（上方和下降中）允许物料的范围
                    {
                        /*机械臂下降*/
                        if (!Ready_to_Grab)
                        {
                    ...
                    else if (Ready_to_Grab && armStepper.onPos_state && fabs(Move_X_Grab) < 10 && Move_Y_Grab > -8)
                    ...
                    if (fabs(Move_X_Grab) < 10 && fabs(Move_Y_Grab) < 10)
                    {
                        ...
                        { // 识别准确
                            ...
                            if (Current_Time - Previous_Time > 500)
                            {
                                ...
                                OPS9.Update_XY(-(Task_Circle[0].y-ARM_TO_CAR_CENTER_DIS-Arm_Zero_Length/10.0-STEPPER_GRIPPER[Green]/10.0),
                                                Task_Circle[0].x);//更新ops坐标
                                // OPS9.Update_X(-(Task_Circle[0].y-ARM_TO_CAR_CENTER_DIS-Arm_Zero_Length/10.0-STEPPER_GRIPPER[Green]/10.0));//更新ops坐标
                                // delay(10);
                                // OPS9.Update_Y(Task_Circle[0].x);//更新ops坐标
                                // delay(10);
                    ...
                    if (fabs(Move_X_Grab) < 10 && fabs(Move_Y_Grab) < 10)
                    {
                        ...
                        { // 识别准确
                            ...
                            if (Current_Time - Previous_Time > 500)
                            {
                                ...
                                // OPS9.Update_Y(Task_Circle[1].x-ARM_TO_CAR_CENTER_DIS-Arm_Zero_Length/10.0-STEPPER_GRIPPER[Green]/10.0);//更新ops坐标
                                // delay(10);
                                // OPS9.Update_X(-Task_Circle[1].y);//更新ops坐标
                                // delay(10);
                                MaixCam.Maix_Detect(Blue);
                                // 机械臂两个自由度PID初始化
                                ARM_PID.PID_Init();
    ...
    case Move:
        ...
        if (CarState == Rotate_Complete)
        {
            ...
            case 11:
                ...
                break;
            default:
                if (MoveIndex == 6 || MoveIndex == 8) // 任务前一个点位
                {
                    armBaseStepper.setAngle(ARM_BASE_STEPPER_ANGLE[2]);
                    MaixCam.Maix_Detect(Green);
                }
